...
void HelloWorld::addEndLine() {
	...
	auto b = Block::createWithArgs(Color3B::GREEN, visibleSize, "You Win", 32, Color4B::BLACK);
	gameLayer->addChild(b);
	b->setLineIndex(4);
}
...
void HelloWorld::addNormalLine(int lineIndex) {
	...
	for (int i = 0; i < 4; i++) {
		b = Block::createWithArgs(blackIndex==i?Color3B::BLACK:Color3B::WHITE, Size(visibleSize.width/4-1, visibleSize.height/4-1)
			, "", 20, Color4B::BLACK);
